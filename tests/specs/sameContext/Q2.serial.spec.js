...
test.describe.serial("Test Suite-2-Q2: Login with standard_user, Add and Purchase any 3 product ", ()=>{
    ...
    test("TCS-Q2.1: Login with ‘standard_user’ and verify user login", async()=>{
    ...
    test("TCS-Q2.2: Reset the App State", async()=>{
        await menuAction.clickMenuOptions('reset');
    });

    test("TCS-Q2.3: Add any three items to the cart", async()=>{
    ...
    test("TCS-Q2.4: Navigate to the checkout page", async()=>{
    ...
    test("TCS-Q2.5: Enter credentials in Checkout Information page.", async()=>{
    ...
    test(`TCS-Q2.6: Verify that ${ProductToAdd} itmes have been added to the cart`, async ()=>{
        totalAddedProducts = await cartAction.getNumberOfAddedProducts();
        expect(totalAddedProducts).toEqual(ProductToAdd);
        console.log(`Number of product added into the cart: ${totalAddedProducts}`);
    });

    test("TCS-Q2.7: Verify that only the added product names are present in the cart", async ()=>{
    ...
    test("TCS-Q2.8: Verify that price of individual added product matches in the cart", async ()=>{
    ...
    test("TCS-Q2.9: Verify the sum of individual added product price matches with cart total price", async ()=>{
    ...
    test("TCS-Q2.10: Finish the purchase journey and verify the successful order message", async ()=>{
    ...
    test("TCS-Q2.11:  Reset the App State and Logout", async()=>{
